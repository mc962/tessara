...
- `tessera-core`: `favicon`, `apple`, `android`, `opengraph` asset groups, plus
  ...
  canvas compositing.
- `tessera-cli`: `tessera generate <source> --preset <web|pwa|social|everything>`,
  runs entirely locally against `tessera-core`. `tessera presets` lists presets.
...
The CLI becomes a dual-mode client for `tessera-core`, not just a local
wrapper:

- **Local mode** (current behavior): `tessera generate ...` calls
  `tessera-core` directly in-process. Requires Pillow/CairoSVG installed
  locally.
- **Remote mode**: commands prefixed with `web` (e.g. `tessera web generate
  ...`) send the request to a configured `tessera-server` instead — same
...
Implementation order: finish `tessera-server`'s API first, confirm it with
real round-trips (including error cases), then add the `web`-prefixed
commands to `tessera-cli` against the settled contract.

## Later: server-side admin CLI

`tessera-server` will eventually need basic user provisioning (accounts,
API access, etc.), backed by a real database with migrations. That brings a
small on-server admin CLI — separate from `tessera-cli`, which stays the
...
Leaning towards: a submodule inside `tessera-server` (e.g.
`tessera_server.cli`, exposed via its own `[project.scripts]` entry) rather
