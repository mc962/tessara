"""Tests for tessara_server.data.repository.api_key_repository."""
...
class TestVerifyKey:
    ...
    async def test_too_short_returns_none(self, mock_db):
        result = await api_key_repository.verify_key(mock_db, "tsa_")
        assert result is None
    ...
    async def test_no_matching_prefix_returns_none(self, mock_db):
        ...
        result = await api_key_repository.verify_key(mock_db, "tsa_" + "a" * 64)
        assert result is None
