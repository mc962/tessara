...
class TestPostLogin:
    def test_non_superuser_key_logs_in(self, unauthed_client):
        with patch(
            "tessera_server.data.repository.api_key_repository.verify_key",
            new=AsyncMock(return_value=_mock_key(is_superuser=False)),
        ):
            resp = unauthed_client.post(
                "/admin/login", data={"api_key": "tsr_x"}, follow_redirects=False
            )
        assert resp.status_code == 303
        assert "tessera_session" in resp.cookies
    ...
    def test_next_param_is_respected(self, unauthed_client):
        with patch(
            ...
            resp = unauthed_client.post(
                "/admin/login",
                data={"api_key": "tsr_x", "next": "/admin/api-keys"},
