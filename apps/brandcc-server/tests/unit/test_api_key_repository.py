"""Tests for brandcc_server.data.repository.api_key_repository."""
...
class TestGenerateApiKey:
    def test_has_bcc_prefix(self):
        key = api_key_repository.generate_api_key()
        assert key.startswith("bcc_")
...
class TestVerifyKey:
    ...
    async def test_too_short_returns_none(self, mock_db):
        result = await api_key_repository.verify_key(mock_db, "bcc_")
        assert result is None
    ...
    async def test_no_matching_prefix_returns_none(self, mock_db):
        ...
        result = await api_key_repository.verify_key(mock_db, "bcc_" + "a" * 64)
        assert result is None

    @pytest.mark.asyncio
    async def test_hash_mismatch_returns_none(self, mock_db):
        from brandcc_server.utility.security import hash_secure_value
