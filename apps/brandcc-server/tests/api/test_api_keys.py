...
from brandcc_server.data.model.api_key import ApiKey


def _mock_key(
    id: int = 1,
    name: str = "test",
    key_prefix: str = "bcc_1234",
...
class TestListApiKeys:
    ...
    def test_returns_empty_list(self, api_client):
        with patch(
            "brandcc_server.data.repository.api_key_repository.list_all",
...
class TestCreateApiKey:
    ...
    def test_creates_key_returns_201(self, api_client):
        ...
        with patch(
            "brandcc_server.data.repository.api_key_repository.create",
            new=AsyncMock(return_value=(created, "bcc_plaintextkey")),
        ):
            resp = api_client.post("/api/api-keys", json={"name": "new-key"})
        assert resp.status_code == 201
        body = resp.json()
        assert body["name"] == "new-key"
        assert body["key"] == "bcc_plaintextkey"
...
class TestToggleApiKey:
    def test_key_not_found_returns_404(self, api_client):
        with patch(
            "brandcc_server.data.repository.api_key_repository.get_by_id",
    ...
    def test_toggles_and_returns_key(self, api_client):
        ...
        with (
            ...
            patch(
                "brandcc_server.data.repository.api_key_repository.toggle_active",
                new=AsyncMock(),
            ),
        ):
...
class TestDeleteApiKey:
    ...
    def test_deletes_key_returns_204(self, api_client):
        ...
        with (
            ...
            patch(
                "brandcc_server.data.repository.api_key_repository.delete",
                new=AsyncMock(),
            ),
        ):
