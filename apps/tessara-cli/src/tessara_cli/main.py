...
from tessara_cli.config import resolve_server_config
from tessara_cli.presets import PRESETS, describe_unknown_preset
from tessara_cli.remote import RemoteGenerationError, extract_zip, generate_remote

app = typer.Typer(help="tessara: generate platform-specific brand assets from a source image.")
web_app = typer.Typer(help="Commands that talk to a remote tessara-server instead of running locally.")
...
def generate(
    ...
    try:
        ...
        # ImportError: tessara-core itself isn't installed (no [local] extra).
        # OSError: tessara-core is installed but cairosvg's native dependency,
        # libcairo2, isn't on the system — common on a stripped-down Pi that
        # only has the pip wheels, not the apt package.
        typer.secho(
            "Local generation needs tessara-core, which isn't fully available:\n"
            f"  {exc}\n"
            "Install it with: pip install tessara-cli[local]\n"
...
def web_generate(
    ...
    server: Annotated[
        ...
        typer.Option(
            "--server",
            envvar="TESSARA_SERVER_URL",
            help="tessara-server base URL. Falls back to the config file, then a prompt.",
        ),
    ] = None,
    api_key: Annotated[
        str | None,
        typer.Option(
            "--api-key",
            envvar="TESSARA_API_KEY",
            help="API key, sent as a Bearer token. Falls back to the config file, then a prompt.",
        ),
    ] = None,
) -> None:
    """Generate brand assets for SOURCE by sending it to a remote tessara-server."""
    if preset not in PRESETS:
        typer.secho(describe_unknown_preset(preset), fg=typer.colors.RED)
        raise typer.Exit(code=1)
