...
from tessera_cli.config import ServerConfig
from tessera_cli.main import app
from tessera_cli.remote import RemoteGenerationError
from tessera_cli.presets import PRESETS
...
class TestGenerate:
    ...
    def test_missing_tessera_core_gives_friendly_message(
        ...
        assert result.exit_code == 1
        assert "tessera-cli[local]" in result.stdout
...
class TestWebGenerate:
    ...
    def test_successful_run_writes_files_and_reports_them(
        ...
        import tessera_cli.main as main_module
